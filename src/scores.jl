...
function Base.iterate(query::ChainScoreQuery)
    ...
    score_names = isempty(query.requested_scores) ?
        collect(keys(chain_data.step_values[1])) : query.requested_scores
...
function eval_score_on_districts(
    ...
    return [
        eval_score_on_district(graph, partition, score, d) for d in districts
    ]
end
...
function get_score_by_name(chain_data::ChainScoreData, score_name::String)
    index = findfirst(
        s -> !ismissing(s.name) && s.name == score_name,
        chain_data.scores,
    )
    if index == nothing
...
function get_score_values(
    ...
    init_vals = nested ? all_scores[1][nested_key][score.name] :
        all_scores[1][score.name]
    num_districts = length(init_vals)
    # create a matrix that is (num states of chain, num districts) to record
    # values of score
    score_table =
        Array{typeof(init_vals[1]),2}(undef, length(all_scores), num_districts)
    # we don't want to alter the data in all_scores
...
function get_score_values(
    ...
    return deepcopy([
        all_scores[i][nested_key][score.name] for i = 1:num_states
    ])
end
...
function get_score_values(
    ...
    fetch_vals =
        s -> get_score_values(all_scores, s, nested_key = composite.name)
...
function get_score_values(chain_data::ChainScoreData, score_name::String)
    ...
    return get_score_values(
        chain_data.step_values,
        score,
    ...
    open(filename, "w") do f
        ...
        for score_name in score_names
            ...
            if score isa DistrictScore || score isa DistrictAggregate
                ...
                district_columns =
                    ["$(score_name)_$(i)" for i = 1:num_districts]
...
type String.
"""
function coerce_attribute_type!(
    graph::BaseGraph,
    key::String,
    new_type::DataType,
)
    for node = 1:graph.num_nodes
        if graph.attributes[node][key] isa String
            graph.attributes[node][key] =
                parse(new_type, graph.attributes[node][key])
            @info string(
                "Key ",
                key,
