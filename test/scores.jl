...
@testset "Score tests" begin
    ...
    @testset "eval_score_on_district()" begin
        ...
        @test_throws ArgumentError eval_score_on_district(
            graph,
            partition,
            wrong_args_score,
            1,
    ...
    @testset "eval_score_on_partition()" begin
        ...
        @test_throws ArgumentError eval_score_on_partition(
            graph,
            partition,
            wrong_args_score,
        )
        ...
        group_score =
            CompositeScore("group", Array{AbstractScore,1}([race_gap, cut_edges_score]))
    ...
    @testset "score_initial_partition()" begin
        ...
        @test score_vals["votes"] ==
              Dict{}("electionD" => [6, 6, 6, 6], "electionR" => [6, 6, 6, 6])
    ...
    @testset "get_scores_at_step()" begin
        ...
        parsed_scores =
            get_scores_at_step(chain_data, 0, score_names = all_score_names[1:2])
        ...
        parsed_scores =
            get_scores_at_step(chain_data, 1, score_names = all_score_names[3:5])
    ...
    @testset "save_scores_to_json()" begin
        ...
        for i = 1:length(step_vals)
            @test step_vals[i]["purple"] == complete_score_vals[i]["purple"]
            @test step_vals[i]["cut_edges"] == complete_score_vals[i]["cut_edges"]
            @test step_vals[i]["votes"]["electionD"] ==
                  complete_score_vals[i]["votes"]["electionD"]
            @test step_vals[i]["votes"]["electionR"] ==
                  complete_score_vals[i]["votes"]["electionR"]
        end
        close(tio)
    end
    ...
    @testset "Type Coercions" begin
        ...
        with_logger(NullLogger()) do # this suppresses the info msg during testing
            @test_logs (
                :info,
                "The str_attr attribute was of type String, ",
